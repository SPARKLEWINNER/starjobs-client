...
context('Client should be able to Create a Gig', () => {
  ...
  it('Should redirect to Pending Category after creating a gig', () => {
    ...
    cy.get('#noConfirmEndShiftButton').click({timeout: 10000})
    cy.get('#hoursLate', {timeout: 5000}).type('0.30')
    cy.get('#confirmEndShiftButton').contains('Submit and Confirm End Shift').click({timeout: 10000})
